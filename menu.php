<?php
    ...
    $conexao = new Conexao();
    $sql = "SELECT subscriptions.id, subscriptions.site, subscriptions.title, user_subscription.id ";
    ...
    if($conexao) {
        while ($rs = $conexao->MostrarResultados()) {
            ...
            $title = $rs[2];
            $subs = $rs[3];
        echo "<li id='$id' data-subscription='$subs' data-site='$site' data-title='$title'><i class='icon-rss-sign'></i> $title </li>";
        }
    }